...
#   endif
#endif
  fips_mode = 0;
  /* need to set env externally */
  /* setenv("ICC_ALLOW_INDUCED","1",1); */
  ...
  */
    if (!error)
    {
      ...
      {
        tfailure = icc_failure - 1000;
        if (ICC_OK != ICC_SetValue(ICC_ctx, status, ICC_INDUCED_FAILURE, (const void *)&tfailure))
        {
          rv = ICC_OSSL_FAILURE;
        }
        check_status(status, __FILE__, __LINE__);
