...
const char *TimeMark();
void TRACE_START(char *source, char *application);
void TRACE_END();
...
void TRACE_START(char *source, char *application)
{
  ...
  if(NULL != logfile) {
    ...
    if(NULL != logfile) {
      fprintf(logfile,"%-16s:%-16s:%-8d,%1s:%s %s %s %s\n",TimeMark(),__FILE__,mypid(),FIPS_TAG,application,source,platform,trc_buffer);
      fprintf(logfile,"%-16s:%-16s:%-8d,%1s,CLOCKS_PER_SEC=%ld\n",TimeMark(),__FILE__,mypid(),FIPS_TAG,(long)CLOCKS_PER_SEC);
      fflush(logfile);
    }  
  }
}
...
void TRACE_END()
{
  ...
  if(NULL != logfile) {
    TimeStamp(buffer);
    fprintf(logfile,"%-16s:%-16s:%-8d:%1s,%s\n",TimeMark(),__FILE__,mypid(),FIPS_TAG,buffer);
    fprintf(logfile,"%-16s:%-16s:%-8d:%1s:<TRACE ENDS>\n\n",TimeMark(),__FILE__,mypid(),FIPS_TAG);
    if(logfile != stderr) {
      fclose(logfile);
    }
    logfile = NULL;
