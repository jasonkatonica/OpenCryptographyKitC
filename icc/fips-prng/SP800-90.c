		...
		unsigned char *Adata,unsigned int Adatal)
{
  ...
  case SP800_90RUN:
    ...
    */
    ...
    t = ((((
	    ictx->CallCount[0] 
	    * 256 + ictx->CallCount[1]) 
	   * 256) + ictx->CallCount[2])
	 * 256)+ ictx->CallCount[3];
      ...
      */
        ...
        */
          ...
          if (NotZero(ictx->K, MAX_K) || NotZero(ictx->V, MAX_V) ||
              ...
              (ictx->minEnt != 0) || (ictx->CallCount[0] != 0) ||
              (ictx->CallCount[1] != 0) || (ictx->CallCount[2] != 0) ||
              (ictx->CallCount[3] != 0) || (ictx->ctx.cctx != NULL))
          {
...
PRNG *get_RNGbyname(const char *algname, int fips) {
  ...
  for (i = 0; PRNG_list[i] != NULL; i++) {
    ...
    if (strcasecmp(algname, PRNG_list[i]->prngname) == 0) {
      ...
      */
       if (!hw_available && (NULL != strstr(PRNG_list[i]->prngname, "ALT4"))) {
          break;
       }
