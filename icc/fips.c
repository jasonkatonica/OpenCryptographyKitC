...
extern int rerunSelfTest;  /*!< Used to trigger a self test if an RNG failover occurred
                                However that can occur during self test so block that
                               */
/**
   @brief
   Standard 'min' macro
...
{
  return insecure_rand_bytes(buf,num);
}
...
static int iccTestTLS_KDF(ICC_STATUS *stat,const EVP_MD *digest, unsigned char *secret,int seclen,unsigned char *seed,int seedlen,unsigned char *expected,int explen)
{
  ...
  if(NULL != kctx) {
    ...
    iccCheckKnownAnswer(out,explen,expected,explen,stat,__FILE__,__LINE__,"TLS KDF known answer failed",""); 
  }
  OUTRC(stat->majRC);       
  return stat->majRC;
}
...
/** @brief Known answer check for asymetric keys
    ...
    @param outL length of signature
    @param flags padding mode for RSA (PSS uses random data)
    @param error if non 0 this will be used to force a failure in this path (ICC_INDUCED_FAILURE)
    @return ICC_OK or ICC_ERROR
...
int iccSetRNGFIPS(ICClib *iccLib, ICC_STATUS *icc_stat, void * seedB, int seedL)
{
  int evpRC=1;
    ...
    if (rngICCRand == NULL) {
      ...
      */
      ...
      if (ICC_OK == icc_stat->majRC) {
        ...
        MARK("SetRNGFIPS", "Read 80 bytes from RNG 1");
        ...
        MARK("SetRNGFIPS", "Read 80 bytes from RNG 2");
        if (ICC_OSSL_SUCCESS != fips_rand_bytes(buffer, 80)) {
          SetStatusLn(NULL, &(Global.status), ICC_ERROR | ICC_FATAL,
                      ICC_LIBRARY_VERIFICATION_FAILED, "RNG failure", __FILE__,
                      __LINE__);
...
void iccDoKnownAnswer(ICClib * iccLib, ICC_STATUS * icc_stat) {
   ...
   * keys B233 */
    ...
    */
    ...
    {
      /** \known Test: TLS1_prf */
      static const char TLS_seed[5] = {0x73, 0x65, 0x65, 0x64, 0x00};
      static const char TLS_secret[7] = {0x73, 0x65, 0x63, 0x72, 0x65, 0x74, 0x00};
      const EVP_MD *md = NULL;
      md = EVP_get_digestbyname("SHA256");
      /** \induced 160 TLS1 kdf mess up the seed */
      memcpy(&ibuf[0], TLS_seed, 5);
      if (160 == icc_failure)
      {
        ibuf[0] = ~ibuf[0];
      }
      /** \induced 161 TLS1 kdf mess up the secret */
      memcpy(&ibuf[16], TLS_secret, 7);
      if (161 == icc_failure)
      {
        ibuf[18] = ~ibuf[18];
      }
